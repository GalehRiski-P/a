...
//   3. Build CLEAN PDF (Surat + CV + Ijazah full teks) untuk backup pribadi
//   4. Build TARGET PDF (Surat img + CV teks + Ijazah img + watermark + encrypt)
//   5. Kirim ke target via Email/WhatsApp
//   6. Jika sukses: kirim CLEAN ke email pribadi + update sheet + hapus file
//   7. Watermark: hanya nama perusahaan
...
const BOT_CONFIG = {
    ...
    FLATTEN_LETTER: true,
    FLATTEN_CV: false,           // CV tetap teks: hyperlink & copy aktif
    FLATTEN_IJAZAH: true,
    ...
    // 'full'    → nama + perusahaan + kontak + timestamp + trackingID
...
function safeUnlink(p) {
    try {
        if (p && fs.existsSync(p)) { fs.unlinkSync(p); return true; }
    } catch {}
    return false;
}
...
function initializeFolders() {
    ['output', 'output/final', 'output/preview', 'templates', 'cv', 'temp', 'temp/cache'].forEach(dir => {
...
// ============================================================
// CACHE RASTERIZED IJAZAH (hemat CPU — ijazah sama tiap target)
// ============================================================
...
async function getRasterizedIjazah() {
    ...
    if (_cachedIjazahRasterPath && isUsableFile(_cachedIjazahRasterPath)) {
        return _cachedIjazahRasterPath;
    }
    ...
    const diskCachePath = path.join(
        cacheDir,
        `ijazah_dpi${BOT_CONFIG.FLATTEN_DPI}_${BOT_CONFIG.FLATTEN_FORMAT}.pdf`
    );

    if (BOT_CONFIG.CACHE_RASTERIZED_IJAZAH && isUsableFile(diskCachePath)) {
        console.log(chalk.gray(`   💾 Cache ijazah rasterized (disk)`));
        _cachedIjazahRasterPath = diskCachePath;
        return diskCachePath;
    }

    console.log(chalk.cyan(`   🖼️  Rasterize Ijazah (DPI ${BOT_CONFIG.FLATTEN_DPI})...`));
    const res = await rasterizePdfToImagePdf(
        IJAZAH_PATH, diskCachePath,
        BOT_CONFIG.FLATTEN_DPI, BOT_CONFIG.FLATTEN_FORMAT, BOT_CONFIG.FLATTEN_JPEG_QUALITY
    );
    if (!res.success) {
        console.log(chalk.yellow(`   ⚠️ Gagal rasterize ijazah: ${res.error}`));
        return null;
    }
    console.log(chalk.gray(`   ↳ Ijazah: ${res.pages} halaman (di-cache)`));
    _cachedIjazahRasterPath = diskCachePath;
    return diskCachePath;
}
...
// ============================================================
// MERGE PDF — URUTAN FIXED: Surat → CV → Ijazah
...
// ============================================================
// ENCRYPT PDF — block print, ALLOW copy & annotation
// ============================================================
async function encryptPdf(inputPath, outputPath, options = {}) {
    ...
    permissions += 512;
    if (!blockPrint) permissions += 4 + 2048;
    if (allowCopy) permissions += 16;
    if (allowAnnotation) permissions += 32;
...
// ============================================================
// BUILD CLEAN PDF — semua teks (no watermark, no protect)
// Untuk backup pribadi + print wawancara
// ============================================================
async function buildCleanPdf(letterPath, target, safeName) {
    const cleanPath = path.join(
        TEMP_DIR,
        `clean_${safeName}_${Date.now()}_${Math.random().toString(36).slice(2, 6)}.pdf`
    );

    const cvPath = isUsableFile(BOT_CONFIG.CV_PATH) ? BOT_CONFIG.CV_PATH : null;
    const ijazahPath = isUsableFile(IJAZAH_PATH) ? IJAZAH_PATH : null;

    const res = await mergePdfsFixed(
        { letterPath, cvPath, ijazahPath },
        cleanPath
    );

    if (!res.success) {
        console.log(chalk.yellow(`   ⚠️ Build clean PDF gagal: ${res.reason}`));
        return null;
    }

    const parts = res.addedOrder.map(k => `${k}:${res.pageCounts[k]}h`).join(' + ');
    console.log(chalk.gray(`   📄 Clean PDF (no watermark, no protect): ${parts} → ${res.totalPages}h`));
    return cleanPath;
}

// ============================================================
// EMAIL TRANSPORTER (reuse biar hemat)
// ============================================================
let _emailTransporterCache = null;
function getEmailTransporter() {
    if (_emailTransporterCache) return _emailTransporterCache;
    _emailTransporterCache = nodemailer.createTransport({
        service: 'gmail',
        auth: {
            type: 'OAuth2',
...
async function ensureEmailAuth() {
    ...
    if (!process.env.GOOGLE_REFRESH_TOKEN) {
        const e = new Error('REFRESH_TOKEN hilang');
...
// ============================================================
async function attemptSendEmail(contact, company, position, letterPath, customSubject = '', letterFileName = '') {
...
async function sendEmail(contact, company, position, letterPath, customSubject = '', letterFileName = '') {
    let attempt = 0;
    while (true) {
        attempt++;
        try {
            console.log(chalk.gray(`   ⏳ Mengirim email ke ${contact}...`));
            await attemptSendEmail(contact, company, position, letterPath, customSubject, letterFileName);
...
// ============================================================
// EMAIL BACKUP — ke galehforjob@gmail.com (tanpa body)
// ============================================================
async function sendBackupEmail(position, cleanPath) {
    ...
    try {
        ...
        await transporter.sendMail({
            ...
            text: '',   // tanpa body
            attachments: [
                { filename: fileName, path: cleanPath },
            ],
        });
        ...
        console.error(chalk.red(`❌ Gagal kirim backup: ${e.message}`));
        return { success: false, reason: e.message };
    }
}

// ============================================================
// WHATSAPP — KIRIM KE TARGET
// ============================================================
...
// ============================================================
// MERGE FINAL — TARGET PDF (watermark + stamp + encrypt)
// Rasterize Surat & Ijazah (parallel), CV tetap teks
// ============================================================
async function getOrCreateMergedPdf(letterPath, target, safeName) {
    ...
    if (BOT_CONFIG.FLATTEN_LETTER && HAS_GHOSTSCRIPT) {
        ...
        tasks.push(
            rasterizePdfToImagePdf(
                letterPath, rastPath,
                BOT_CONFIG.FLATTEN_DPI, BOT_CONFIG.FLATTEN_FORMAT, BOT_CONFIG.FLATTEN_JPEG_QUALITY
            ).then(res => {
                if (res.success) {
                    preparedLetter = rastPath;
                    console.log(chalk.gray(`   ↳ Surat: ${res.pages}h jadi gambar`));
                } else {
    ...
    if (BOT_CONFIG.FLATTEN_IJAZAH && hasIjazah && HAS_GHOSTSCRIPT) {
        tasks.push(
            getRasterizedIjazah().then(p => {
                if (p) preparedIjazah = p;
            })
        );
    }

    if (tasks.length) {
        console.log(chalk.cyan(`   🖼️  Rasterize (parallel, DPI ${BOT_CONFIG.FLATTEN_DPI})...`));
        await Promise.all(tasks);
    }

    if (hasCv && !BOT_CONFIG.FLATTEN_CV) {
        console.log(chalk.cyan(`   📄 CV: teks (hyperlink & copy aktif)`));
    }

    // === STEP 2: Merge Surat → CV → Ijazah ===
    ...
    if (!mergeRes.success) {
        ...
        if (letterDir === officialOutputDir) safeUnlink(letterPath);
    ...
    if (BOT_CONFIG.WATERMARK_ENABLED) {
        ...
        if (wmRes.success) {
            ...
            console.log(chalk.gray(`   ↳ Watermark ${wmRes.pages}h`));
        } else {
    ...
    if (HAS_GHOSTSCRIPT) {
        ...
        if (encRes.success) {
            ...
            console.log(chalk.yellow(`   ⚠️ Encrypt gagal: ${encRes.error} — tanpa encrypt`));
            safeUnlink(finalPath);
            try { fs.renameSync(stampedPath, finalPath); finalUsedPath = finalPath; } catch {}
        }
    } else {
        safeUnlink(finalPath);
        try { fs.renameSync(stampedPath, finalPath); finalUsedPath = finalPath; } catch {}
    ...
    // Cleanup rasterized letter (tidak dibutuhkan lagi)
    if (preparedLetter !== letterPath && path.resolve(path.dirname(preparedLetter)) === path.resolve(TEMP_DIR)) {
        safeUnlink(preparedLetter);
    }
    ...
    console.log(chalk.green(`   ✅ Final: ${finalFileName} (${stampResult.totalPages}h, ${tagList.join(', ')})`));
...
async function main() {
    ...
    console.log(chalk.bold.cyan(`   Watermark: ${BOT_CONFIG.WATERMARK_ENABLED ? `AKTIF (mode: ${BOT_CONFIG.WATERMARK_MODE})` : 'off'}`));
    console.log(chalk.bold.cyan(`   Print: ${BOT_CONFIG.BLOCK_PRINT ? 'BLOCKED' : 'OK'} | ` +
        `Copy: ${BOT_CONFIG.ALLOW_COPY ? 'OK' : 'BLOCKED'} | ` +
        `Annot: ${BOT_CONFIG.ALLOW_ANNOTATION ? 'OK' : 'BLOCKED'}`));
    console.log(chalk.bold.cyan(`   Backup: ${BOT_CONFIG.BACKUP_ENABLED ? `→ ${BOT_CONFIG.BACKUP_EMAIL}` : 'off'}`));
    console.log(chalk.bold.cyan(`   Cleanup: ${BOT_CONFIG.DELETE_AFTER_SENT ? 'ON (setelah sent)' : 'off'}`));
    ...
    let backupSentCount = 0, deletedCount = 0;
    ...
    for (let i = 0; i < targets.length; i++) {
        ...
        // ---- 2. Build CLEAN PDF (untuk backup pribadi) ----
        let cleanPath = null;
        if (BOT_CONFIG.BACKUP_ENABLED) {
            cleanPath = await buildCleanPdf(letterPath, target, safeName);
        }

        // ---- 3. Build TARGET PDF (watermark + stamp + encrypt) ----
        const mergedInfo = await getOrCreateMergedPdf(letterPath, target, safeName);
        const targetPath = mergedInfo.path;
        const letterFileName = mergedInfo.fileName || path.basename(targetPath);
        ...
        if (isEmail(target.contact)) {
            ...
            sendSuccess = await sendEmail(
                target.contact, target.company, target.position,
                targetPath, target.subject, letterFileName
            );
        } else if (waState) {
            console.log(chalk.gray(`   📄 File  : ${letterFileName}`));
            sendSuccess = await sendWhatsApp(
                waState, target.contact, target.company, target.position,
                targetPath, letterFileName
            );
        }
        ...
        if (sendSuccess) {
            ...
            // 5a. Backup ke email pribadi
            if (BOT_CONFIG.BACKUP_ENABLED && cleanPath) {
                const backupRes = await sendBackupEmail(target.position, cleanPath);
                backupOk = backupRes.success;
                if (backupOk) backupSentCount++;
                else console.log(chalk.yellow(`   ⚠️ Backup gagal: ${backupRes.reason} — clean file disimpan`));
            }

            // 5b. Update sheet
            await updateGoogleSheetStatus(target.company, target.position, 'sent', targetPath);
            successCount++;

            // 5c. Cleanup
            if (BOT_CONFIG.DELETE_AFTER_SENT) {
                if (safeUnlink(targetPath)) {
                    deletedCount++;
                    console.log(chalk.gray(`   🗑️  Target dihapus: ${path.basename(targetPath)}`));
                }
                if (BOT_CONFIG.BACKUP_ENABLED && cleanPath) {
                    if (backupOk) {
                        if (safeUnlink(cleanPath)) {
                            deletedCount++;
                            console.log(chalk.gray(`   🗑️  Clean dihapus: ${path.basename(cleanPath)}`));
                        }
                    } else {
            ...
            console.log(chalk.gray(`   ℹ️  Target disimpan: ${targetPath}`));
            await updateGoogleSheetStatus(target.company, target.position, 'failed', targetPath);
            failCount++;
        }
    ...
    console.log(chalk.cyan(`📤 Backup terkirim: ${backupSentCount}`));
    console.log(chalk.cyan(`🗑️  File dihapus: ${deletedCount}`));
